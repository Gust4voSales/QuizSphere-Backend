...
async function createNotification(friendId, quizId, senderName) {
  try {
...
module.exports = {
    ...
    try {
      ...
      const user = await User.findById(userId, "sharedQuizzes")
        ...
        .populate({
          ...
          select:
            "quizTitle tags questionsLength time author likes likeCounter private",
          populate: {
            path: "author",
            model: "User",
            select: "userName",
      ...
      user.sharedQuizzes.map((sharedQuizObj, index) => {
        if (sharedQuizObj.quiz === null) return;
      ...
      return res
        .status(400)
        .json({ error: "Não foi possível listar os quizzes recebidos." });
    }
  },
  ...
  async store(req, res) {
    try {
      ...
      for (let friendId of friendsIds) {
        ...
        if (quiz.private && userId != quiz.author) {
          return res.status(400).json({
            error: "Apenas o autor pode compartilhar um quiz privado",
          });
        }
        ...
        // add notification
        await createNotification(friendId, quizId, senderName);
        ...
        if (ownerSocketRecipient) {
          req.io
            .to(ownerSocketRecipient)
            .emit("new_activity", { type: "shared" });
        }
      }
      ...
      return res
        .status(400)
        .json({ error: "Não foi possível compartilhar o quiz." });
    }
  },
};
